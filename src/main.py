...
def report_three_combined(ingram_sales_df: pl.DataFrame,sage_sales_df: pl.DataFrame):
    #Order and standardize data
    column_order = ['ISBN', 'YEAR', 'MONTH', 'TITLE', 'NAMECUST', 'NETUNITS', 'NETAMT', 'TUTTLE_SALES_CATEGORY']
    ingram_sales_df = ingram_sales_df.select(column_order)
    sage_sales_df = sage_sales_df.select(column_order)
    ...
    ingram_sales_df = ingram_sales_df.with_columns(
        ...
        pl.col("TUTTLE_SALES_CATEOGRY").cast(pl.Utf8).str.strip_chars()
    )
    sage_sales_df = sage_sales_df.with_columns(
        pl.col("ISBN").cast(pl.Utf8).str.strip_chars(),
        pl.col("YEAR").cast(pl.Int64),
    ...
    customers_in_both = sage_customers & ingram_customers #set addition
    ...
    grouping_keys = ["ISBN","TITLE","NAMECUST","TUTTLE_SALES_CATEOGRY"]
    ...
    base_df = combined_df[grouping_keys].unique()
